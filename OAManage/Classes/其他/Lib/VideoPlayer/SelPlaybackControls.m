...
- (void)makeConstraints
{
    ...
    [_fullScreenButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(_bottomControlsBar).mas_equalTo(-10);
        make.bottom.equalTo(_bottomControlsBar);
        make.size.mas_equalTo(CGSizeMake(30, 30));
    }];
    
    [_playTimeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(_bottomControlsBar).offset(8);
        make.width.equalTo(@45);
        make.centerY.equalTo(_bottomControlsBar.mas_centerY);
    }];
    
    [_totalTimeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(_fullScreenButton.mas_left).offset(-8);
        make.width.equalTo(@45);
        make.centerY.equalTo(_bottomControlsBar.mas_centerY);
    }];
...
- (UIButton *)lockBtn{
    if (!_lockBtn) {
        ...
        [_lockBtn setImage:[UIImage imageNamed:@"开锁"] forState:UIControlStateNormal];
        [_lockBtn setImage:[UIImage imageNamed:@"锁屏"] forState:UIControlStateSelected];
        _lockBtn.hidden = YES;
        [_lockBtn addTarget:self action:@selector(openLockAction:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _lockBtn;
}
...
- (UIButton *)backBtn{
    if (!_backBtn) {
        ...
        [_backBtn addTarget:self action:@selector(outFullScreenAndOutShowAction:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _backBtn;
}
...
/**锁屏的按钮的点击事件*/
- (void)openLockAction:(UIButton *)lockBtn{
    lockBtn.selected = !lockBtn.selected;
    if (lockBtn.selected) {
        self.playButton.hidden = YES;
        self.bottomControlsBar.hidden = YES;
        self.backBtn.hidden = YES;
        self.lockBtn.hidden = NO;
    }else{
        self.backBtn.hidden = NO;
       self.playButton.hidden = NO;
       self.bottomControlsBar.hidden = NO;
       self.lockBtn.hidden = NO;
    }
    if (_delegate && [_delegate respondsToSelector:@selector(lockButtonAction:)]) {
        [_delegate lockButtonAction:lockBtn.selected];
    }
}

/**退出并退出全屏*/
- (void)outFullScreenAndOutShowAction:(UIButton *)backBtn{
    if (_delegate && [_delegate respondsToSelector:@selector(outFullScreenAndOutShow)]) {
           [_delegate outFullScreenAndOutShow];
       }
}
