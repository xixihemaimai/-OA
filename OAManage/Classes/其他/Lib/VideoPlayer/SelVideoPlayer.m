...
/**按钮点击事件*/
- (void)lockButtonAction:(BOOL)selected{
    if (selected) {
        _playerConfiguration.shouldAutorotate = NO;
    }else{
        _playerConfiguration.shouldAutorotate = YES;
    }
}
...
- (void)outFullScreenAndOutShow{
    if (_isFullScreen) {
        //退出全屏
        [self _videoZoomOut];
    }else{
        //返回上一个界面
        if ([self.delegate respondsToSelector:@selector(upViewController)]) {
            [self.delegate upViewController];
        }
    }
}
