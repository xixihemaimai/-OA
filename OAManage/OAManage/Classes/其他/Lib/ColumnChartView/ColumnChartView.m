...
- (void)resetLegend {
    ...
    if (self.legendPostion == LegendPositionNone) {// 无图例的时候
        ...
        self.legendBgView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, 40)];
        [self addSubview:self.legendBgView];
        switch (self.legendPostion) {
            case LegendPositionTop:
                self.legendBgView.center = CGPointMake(self.bounds.size.width / 2, 20);
                break;
...
//            colorPoint.layer.masksToBounds = YES;
            ...
            if (self.isColumnGradientColor) {
                ...
                if (_isSingleColumn) {
                    gradientColorArr = @[@"#27C79A",@"#FCC828"];
                }
                ...
                legendGradientLayer.colors = @[(__bridge id)[self colorWithHexString:gradientColorArr[i]].CGColor,
                                               (__bridge id)[self colorWithHexString:gradientColorArr[i]].CGColor];
...
- (void)resetSingleColumn { // 单柱
    ...
    switch (self.legendPostion) {
        case LegendPositionNone:
            ...
            break;
        case LegendPositionTop:
            self.yAxisView.frame = CGRectMake(0, 40, 100, self.bounds.size.height - 40);
            self.scrollView.frame = CGRectMake(100, 20, self.bounds.size.width - 112, self.bounds.size.height - 20);
            break;
        case LegendPositionBottom:
    ...
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(99, 0, 1, self.yAxisView.bounds.size.height - 20)];
    lineView.backgroundColor = [self colorWithHexString:@"#CCCCCC"];
...
//    if ([str intValue] == 1) {
//        str = [NSString stringWithFormat:@"%d",[str intValue] + 9];
//    }else if ([str intValue] == 2){
//        str = [NSString stringWithFormat:@"%d",[str intValue] + 8];
//    }else if ([str intValue] == 3){
//         str = [NSString stringWithFormat:@"%d",[str intValue] + 7];
//    }else if ([str intValue] == 4){
//         str = [NSString stringWithFormat:@"%d",[str intValue] + 6];
//    }
//    else if ([str intValue] == 5){
//         str = [NSString stringWithFormat:@"%d",[str intValue] + 5];
//    }
//    else if ([str intValue] == 6){
//         str = [NSString stringWithFormat:@"%d",[str intValue] + 4];
//    }
//    else if ([str intValue] == 7){
//         str = [NSString stringWithFormat:@"%d",[str intValue] + 3];
//    }
//    else if ([str intValue] == 8){
//         str = [NSString stringWithFormat:@"%d",[str intValue] + 2];
//    }
//    else if ([str intValue] == 9){
//         str = [NSString stringWithFormat:@"%d",[str intValue] + 1];
//    }
    int count = (int)maxNum / [str intValue];
    for (int i = 0; i < [str intValue]; i++) {
        CGFloat lblY = self.yAxisView.bounds.size.height - 28 - i * (lineView.bounds.size.height / [str intValue]);
        UILabel *lblYAxisNum = [[UILabel alloc] initWithFrame:CGRectMake(0, lblY, 100, 16)];
        lblYAxisNum.font = [UIFont systemFontOfSize:10];
        lblYAxisNum.textColor = [self colorWithHexString:@"#CCCCCC"];
        lblYAxisNum.textAlignment = NSTextAlignmentCenter;
//        lblYAxisNum.text = [NSString stringWithFormat:@"%d",i * level];
        
        lblYAxisNum.text = [NSString stringWithFormat:@"%d",i * count];
        
        [self.yAxisView addSubview:lblYAxisNum];
    }
    UILabel *lblMax = [[UILabel alloc] initWithFrame:CGRectMake(0, -8, 100, 16)];
    lblMax.font = [UIFont systemFontOfSize:10];
    lblMax.textColor = [self colorWithHexString:@"#CCCCCC"];
    ...
    xLineView.backgroundColor = [self colorWithHexString:@"#CCCCCC"];
    ...
    if (self.showDataHorizontalLine) {
        for (int i = 0; i < [str intValue]; i++) {
            ...
            lineLayer.strokeColor = [self colorWithHexString:@"#CCCCCC"].CGColor;
            //  设置路径
    ...
    for (int i = 0; i < self.dataArray.count; i++) {
        ...
        lblGroupTitle.font = [UIFont systemFontOfSize:13];
        lblGroupTitle.textColor = [self colorWithHexString:@"#333333"];
        ...
        dataView.center = CGPointMake(groupWidth * i + groupWidth / 2 + 8 * i, dataView.center.y);
        ...
        if (self.isColumnGradientColor) {
            ...
            columnGradientLayer.colors = @[(__bridge id)[self colorWithHexString:self.columnGradientColorArray[i]].CGColor,
                                           (__bridge id)[self colorWithHexString:self.columnGradientColorArray[i]].CGColor];
...
- (void)resetMultiColumns {
    ...
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(55, 0, 1, self.yAxisView.bounds.size.height - 20)];
    lineView.backgroundColor = [self colorWithHexString:@"#CCCCCC"];
    [self.yAxisView addSubview:lineView];
...
//    NSString * str = [[NSString stringWithFormat:@"%ld",(long)maxNum] substringToIndex:1];
    NSString * str = @"10";
//    int level = (int)maxNum / [str intValue]; // 数量级 整十或整百或整千等
//    str = [NSString stringWithFormat:@"%d",level + 5];
//    NSLog(@"------------------------%d",maxValue);
//    NSLog(@"========================%ld",maxNum);
//    NSLog(@"------------------------%d",level);
...
//         str = [NSString stringWithFormat:@"%d",[str intValue] + 1];
//    }
//    NSLog(@"++++++++3232++++++++++++++++=%d",[str intValue]);
    int count = (int)maxNum / [str intValue];
    for (int i = 0; i < [str intValue]; i++) {
        CGFloat lblHeight = self.yAxisView.bounds.size.height - 28 - i * (lineView.bounds.size.height / [str intValue]);
        UILabel *lblYAxisNum = [[UILabel alloc] initWithFrame:CGRectMake(0, lblHeight, 56, 16)];
        lblYAxisNum.font = [UIFont systemFontOfSize:10];
        lblYAxisNum.textColor = [self colorWithHexString:@"#CCCCCC"];
        lblYAxisNum.textAlignment = NSTextAlignmentCenter;
        
//        if ([NSString stringWithFormat:@"%d",maxValue].length == 5 || [NSString stringWithFormat:@"%d",maxValue].length == 6 || [NSString stringWithFormat:@"%d",maxValue].length == 7) {
//            NSLog(@"------------22------------%d",level);
//            level = [[[NSString stringWithFormat:@"%d",level] substringToIndex:[NSString stringWithFormat:@"%d",maxValue].length] intValue];
//            NSLog(@"------------33------------%d",level);
//        }
...
//        lblYAxisNum.text = [NSString stringWithFormat:@"%d",i * level];
    ...
    UILabel *lblMax = [[UILabel alloc] initWithFrame:CGRectMake(0, -8, 56, 16)];
    lblMax.font = [UIFont systemFontOfSize:10];
    lblMax.textColor = [self colorWithHexString:@"#CCCCCC"];
    lblMax.textAlignment = NSTextAlignmentCenter;
    lblMax.text = [NSString stringWithFormat:@"%ld",(long)maxNum];
    ...
    for (int i = 0; i < self.dataArray.count; i++) {
        ...
        for (int j = 0; j < numInEachGroup; j++) {
            ...
            if (self.showDataLabel) {
                ...
                [dataView addSubview:dataLable];
                [dataLable bringSubviewToFront:self.scrollView];
            }
        }
